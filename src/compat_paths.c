...
char **compat_get_default_conf_paths(void)
{
    ...
    if (!paths[0]) {
        paths[0] = "hydrasdr_433.conf";
        if (env_config_home && *env_config_home)
            snprintf(buf, sizeof(buf), "%s%s", env_config_home, "/hydrasdr_433/hydrasdr_433.conf");
        else
            snprintf(buf, sizeof(buf), "%s%s", getenv("HOME"), "/.config/hydrasdr_433/hydrasdr_433.conf");
        paths[1] = buf;
#define STR_VALUE(arg) #arg
#define STR_EXPAND(s)  STR_VALUE(s)
        paths[2] = STR_EXPAND(INSTALL_SYSCONFDIR) "/hydrasdr_433/hydrasdr_433.conf";
        paths[3] = NULL;
    };
    return paths;
}
...
char **compat_get_default_conf_paths(void)
{
    ...
    if (GetModuleFileName(NULL, bufs[0], sizeof(bufs[0]))) {
        ...
        if (last_backslash)
            *last_backslash = '\0';
        strcat_s(bufs[0], sizeof(bufs[0]), "\\hydrasdr_433.conf");
        paths[0] = bufs[0];
    }
    else {
        paths[0] = NULL;
    }
    // Local per user configuration files %LocalAppData% (e.g. Win7: C:\Users\myusername\AppData\Local\hydrasdr_433\hydrasdr_433.conf)
    if (SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, 0, bufs[1]) == S_OK) {
        strcat_s(bufs[1], sizeof(bufs[1]), "\\hydrasdr_433\\hydrasdr_433.conf");
        paths[1] = bufs[1];
    }
    else {
        paths[1] = NULL;
    }
    // Per machine configuration data %ProgramData% (e.g. Win7: C:\ProgramData\hydrasdr_433\hydrasdr_433.conf)
    if (SHGetFolderPath(NULL, CSIDL_COMMON_APPDATA, NULL, 0, bufs[2]) == S_OK) {
        strcat_s(bufs[2], sizeof(bufs[2]), "\\hydrasdr_433\\hydrasdr_433.conf");
        paths[2] = bufs[2];
    }
    else {
        paths[2] = NULL;
