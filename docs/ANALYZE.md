...
Try analyzing each sample with `hydrasdr_433 -A gfile.cu8` to see if there is some real data.
...
hydrasdr_433 processes radio data in multiple stages. You can follow the stages and verify the data at each point.
...
Then grab the signal with hydrasdr_433, e.g. `hydrasdr_433 -f 433.92M -S unknown`
...
Run `hydrasdr_433 -A SAMPLE.cu8` to get an overview of the timings,
or `hydrasdr_433 -w OOK:- SAMPLE.cu8` to see the raw data.
Write the pulses to a file with `hydrasdr_433 -w SAMPLE.ook SAMPLE.cu8`
and visualize the file with https://triq.org/pdv

::: warning
You need to give the sample rate if it's not 250k, look at the file name, e.g. use `hydrasdr_433 -s 1000k -A SAMPLE_1000k.cu8`
:::

For advanced analysis you can also try out SigRok's Pulseview with `hydrasdr_433 -W out.sr SAMPLE.cu8`.
